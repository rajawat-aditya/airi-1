...
const ReactionBar = React.forwardRef((
  {
    className,
    variant,
    size,
  ...
  return (
    <div className="relative inline-block">
      ...
      <motion.button
        ...
        {...props}>
        {renderContent()}
        {showLabel && <span className="ml-1">{(selectedReaction || defaultReaction || reactions[0]).label}</span>}
      </motion.button>
      <AnimatePresence>
        {(isHovering || isMobileOpen) && (
          <motion.div
            ...
            className={cn(
              ...
              "md:w-auto w-50 md:flex-nowrap flex-wrap justify-center",
              popoverClassName
            )}
            onMouseEnter={() => setIsHovering(true)}
            onMouseLeave={() => setIsHovering(false)}>
