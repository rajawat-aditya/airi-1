...
async def browser_search(queries: list[str]): # FEATURE 1 IMPLEMENTED: Accepts list of subqueries
    ...
    # We use async_playwright context manager to handle the browser lifecycle
    async with async_playwright() as p:
        # Launch browser (Playwright manages the executable automatically)
        browser = await p.chromium.launch(headless=False)
        context = await browser.new_context() # Contexts keep sessions isolated and clean
        ...
        for query in queries:
            ...
            # Note: in Playwright, it's wait_until, not waitUntil
        ...
        async def scrape_page(item):
            ...
            try:
                ...
                await new_page.goto(item["link"], wait_until="domcontentloaded")
                await asyncio.sleep(2)
